...
$pages | ForEach-Object {
    ...
    $matches = ([regex]::Matches($pagecontent, [regex]::Escape($find), [Text.RegularExpressions.RegexOptions]::IgnoreCase)).Count
    if($matches -gt 0){
        ...
        Set-Content -Encoding UTF8 -Path $_.FullName -Value $newcontent

        # Console output plus running tally
        Write-Host -ForegroundColor Cyan "Updated $matches occurrences"
        $totaledits = $totaledits + $matches
        $editedpages++
    }
